...
from naff import Button, ButtonStyles, Select, SelectOption, InteractionContext, PartialEmoji, ComponentContext
...
def comp_select(guild_id: int, state: CompState):
    ...
    options = [SelectOption(comp[4], str(comp[0]), comp[5], PartialEmoji(name="➡️")) for comp in comps]
    return Select(options, "contest_chooser", "Pick a contest from your guild.")

async def choose_comp(ctx: InteractionContext, msg: str, state: CompState) -> ComponentContext:
    select = comp_select(ctx.guild_id, state)
    origin = await ctx.send(embed=info_embed("Choose a contest.", f"Pick a contest to {msg}."), components=select)
    ...
    def check(component: Select):
        ...
        return (
            ...
            ctx.author.id == component.context.author.id
        )
    ...
    try: 
        ...
        await used.context.edit_origin(embed=error_embed("Timed out. Please try again, and be quicker this time."), components=select)
    else:
        return used.context
